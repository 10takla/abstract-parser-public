...
pub fn choice_rule(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    fn rec_arm<'a>(
        ...
        if let Some((i, (j, var_ident))) = iter.next() {
            let err_arm = rec_arm(iter, i, choice_error);
    ...
    let matches = rec_arm(&mut iter, Default::default(), &choice_error);
    ...
    quote! {
        ...
        #error_attrs
        #vis struct #choice_error #output_impl #output_where (#(pub abstract_parser::ProductionError<<#error_vars as abstract_parser::TransferRule<__IS>>::Error>),*);

        impl<#(#bounded_generics),*> abstract_parser::TransferRule<__IS> for #ident #type_
        {
