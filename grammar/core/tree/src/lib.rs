...
use crate::grammar::{
    EnumOutput, FieldOutput, FieldTypeOutput, Grammar, StructFieldOutput, StructOutput, VarOutput,
};
            ...
            match fields {
                FieldTypeOutput::Struct(v) => {
                    let iter = v.iter().map(|v| match v {
                        FieldOutput::Struct(StructFieldOutput { ident, value }) => {
                            let ident = to_ident(ident);
                            let value = token(value);
                            quote!(pub #ident: #value)
                        }
                        FieldOutput::Tuple(v) => {
                            let v = token(v);
...
mod grammar {
    ...
    use parsers::chars::{
        ...
        reg_expr_token, InputStreamTrait,
    };
    ...
    #[choice_rule(
        OutputAttrs: #[derive(Debug, Clone, PartialEq)]
        ErrorAttrs: #[derive(Debug, Clone, PartialEq)]
