...
use grammar_extended::{
    parser::{AnyOrParenOutput, SeqOrTokenOutput, quantificator_feature::Comment},
    tree::parser::{
        ChoiceOrQuantificator, CombinatorOutput, Enum as Choice, EnumOutput, ExprOutput, FieldOutput, IdentWithDefineGenericsOrIdentOutput, IgnoredExprVOutput, JoinableOutput, KleeneOutput, PredicativeOutput, QuantificatorOrTokenOrSeqOutput, QuantificatorOrTokenOutput, QuantificatorOutput, RepeatQuantificatorOutput, Struct as Sequence, StructOutput, StructTypeOutput, TokenOutput, TupleItemOutput, TupleStructExprOutput,
        __Struct
    },
};
...
pub fn default_feature_rule<'src>() -> WrapRule<Space<'src>, JoinableRule<Repeat, VecChoiceRule<FeatureRule<'src>>, Space<'src>>, Space<'src>> {
    WrapRule(
        Space::default(),
        JoinableRule {
            rule: VecChoiceRule(
    ...
    while let Some(v) = iter.next() {
        match v {
            ...
            FeatureVOutput::Other(v) => {
                ts.extend(
                features_parse(
                CachedIter::new(InputStreamIter::new(&v.into_iter().map(|v| v.0 .1).collect::<String>()))
                            .full_parse(&WrapRule(
                                Space::default(),
                                JoinableRule {
                                    rule: VecChoiceRule(
                                    features
                                        ...
                                        .map(|v| match v.name {
                                            ...
                                            "squence_rule" => Feature::SequenceRule(SequenceRule::default()),
                                            "quantificator_rule" => Feature::QuantificatorRule(QuantificatorRule::default()),
...
pub fn features_parse<'src, IS: InputStreamTrait<'src>>(features: Vec<<FeatureRule<'src> as TransferRule<IS>>::Output>) -> TokenStream2 {
    features
        .into_iter()
        .filter_map(|v| {
            let name = match &v {
            ...
            Some(match v {
                ...
                _ => {
                    ...
                    quote!(pub use self::#mod_name::{#name, *}; pub mod #mod_name {use super::*; #mod_body})
                }
            })
        })
        .collect()
