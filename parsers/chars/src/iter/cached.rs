...
use crate::{iter::CharsIterTrait, tmp, CharParser, InputStreamTrait, ParseError, TransferRule};
use parser::{CachedIter, Promotable};
...
impl<'src, IS: InputStreamTrait<'src>> CharParser<'src> for CachedIter<IS> {
    fn full_parse<Rule: TransferRule<'src, Self>>(
        ...
        tmp::<Rule, _>(self.parse(rule), &self.iter)
    }
}
