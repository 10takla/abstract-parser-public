...
#![feature(phantom_variance_markers, macro_metavar_expr_concat, trait_alias, const_trait_impl, const_default)]
...
impl<'src> CharParser<'src> for InputStreamIter<'src> {
    fn full_parse<Rule: TransferRule<'src, Self>>(
        ...
        check_full_parse::<Rule, _>(self.parse(rule), self)
    }
}

pub fn check_full_parse<'src, Rule: TransferRule<'src, IS>, IS>(
    res: Result<Rule::Output, ProductionError<Rule::Error>>,
    iter: &impl CharsIterTrait<'src>,
) -> Result<Rule::Output, ParseError<'src, Rule::Output, Rule::Error>> {
    res.map_err(Err)
        .and_then(|parsed| {
